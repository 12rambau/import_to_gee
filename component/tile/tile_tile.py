...
class TileTile(sw.Tile):
    ...
    def __init__(self, m, aoi_model):
        ...
        # inputs
        self.grid_name = v.TextField(label=cm.tile.asset_lbl, v_model=None)

        self.batch_size = sw.NumberField(
            label=cm.tile.nb_batch_lbl, v_model=1, max_=100
        )
    ...
    def create_grid(self, widget, data, event):
        ...
        # check the vars
        if not self.alert.check_input(aoi.name, cm.no_aoi):
            return
        if not self.alert.check_input(grid_batch, cm.no_size):
            return
        if not self.alert.check_input(grid_name, cm.no_name):
            return
        ...
        self.m.addLayer(
            ee.FeatureCollection(asset), {"color": color.accent}, cm.tile.grid_layer
        )
