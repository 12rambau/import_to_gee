...
class DownloadTile(sw.Tile):
    ...
    def load_file(self, widget, data, event):
        ...
        if not self.alert.check_input(myfiles, cm.download.no_file):
            return
        ...
        if self.select_type.v_model == self.SELECT_TYPE[0]:
            ...
            if not all(ext in suffixes for ext in self.SHP_SUFFIX[:3]):
                self.alert.add_msg(
                    cm.download.missing_files.format(", ".join(self.SHP_SUFFIX[:3])),
                    "error",
                )
                return
            ...
            if not all(ext in self.SHP_SUFFIX for ext in suffixes):
                self.alert.add_msg(
                    cm.download.unknown_extention.format(", ".join(self.SHP_SUFFIX)),
                    "error",
                )
                return
